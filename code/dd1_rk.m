...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Xd]=dd1_rk(T_D,Fb,Wibb,attiN,veloN,posiN,Xd,Vd)
  Re=6378137.0;                                      %地球半径（米） 
  f=1/298.257;                                       %地球的椭圆率
...
%       -VW(2,1),VW(1,1),0];
  ...
  k1(1:3,1)=VV*Xd_tmp(1:3,1)+Vnon*Fb+VP*Xd_tmp(8:10,1)+Cbn*Xd_tmp(17:19,1);
  %k1(4:19,1)=FI*Xd_tmp+GI*Vd_tmp;
  k1(4:19,1)=FI*Xd_tmp;
  %%%%%%%%%%%%%%%%%% 龙格库塔的k2系数项%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Xd_tmp=Xd+0.5*T_D*k1;
  %Vd_tmp=Vd;  
  %%%%%%%速度误差的非线性部分%%%%%%%%
  XQ=Xd_tmp(4:7,1);
  Vnon=zeros(3,3);
  ...
  Vnon(3,1)=2*(Q(2,1)*XQ(4,1)+Q(4,1)*XQ(2,1)-Q(1,1)*XQ(3,1)-Q(3,1)*XQ(1,1)-XQ(2,1)*XQ(4,1)+XQ(1,1)*XQ(3,1));
  Vnon(3,2)=2*(Q(1,1)*XQ(2,1)+Q(2,1)*XQ(1,1)+Q(3,1)*XQ(4,1)+Q(4,1)*XQ(3,1)-XQ(3,1)*XQ(4,1)-XQ(1,1)*XQ(2,1));
  k2(1:3,1)=VV*Xd_tmp(1:3,1)+Vnon*Fb+VP*Xd_tmp(8:10,1)+Cbn*Xd_tmp(17:19,1);
  %k2(4:19,1)=FI*Xd_tmp+GI*Vd_tmp; 
  k2(4:19,1)=FI*Xd_tmp; 
  %%%%%%%%%%%%%%%%%% 龙格库塔的k3系数项%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Xd_tmp=Xd+0.5*T_D*k2;
  %Vd_tmp=Vd;  
  %%%%%%%速度误差的非线性部分%%%%%%%%
  XQ=Xd_tmp(4:7,1);
  Vnon=zeros(3,3);
  ...
  Vnon(3,1)=2*(Q(2,1)*XQ(4,1)+Q(4,1)*XQ(2,1)-Q(1,1)*XQ(3,1)-Q(3,1)*XQ(1,1)-XQ(2,1)*XQ(4,1)+XQ(1,1)*XQ(3,1));
  Vnon(3,2)=2*(Q(1,1)*XQ(2,1)+Q(2,1)*XQ(1,1)+Q(3,1)*XQ(4,1)+Q(4,1)*XQ(3,1)-XQ(3,1)*XQ(4,1)-XQ(1,1)*XQ(2,1));
  k3(1:3,1)=VV*Xd_tmp(1:3,1)+Vnon*Fb+VP*Xd_tmp(8:10,1)+Cbn*Xd_tmp(17:19,1);
  %k3(4:19,1)=FI*Xd_tmp+GI*Vd_tmp;  
  k3(4:19,1)=FI*Xd_tmp;
  %%%%%%%%%%%%%%%%%% 龙格库塔的k4系数项%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Xd_tmp=Xd+T_D*k3;
  %Vd_tmp=Vd;  
  ...
  Vnon(3,1)=2*(Q(2,1)*XQ(4,1)+Q(4,1)*XQ(2,1)-Q(1,1)*XQ(3,1)-Q(3,1)*XQ(1,1)-XQ(2,1)*XQ(4,1)+XQ(1,1)*XQ(3,1));
  Vnon(3,2)=2*(Q(1,1)*XQ(2,1)+Q(2,1)*XQ(1,1)+Q(3,1)*XQ(4,1)+Q(4,1)*XQ(3,1)-XQ(3,1)*XQ(4,1)-XQ(1,1)*XQ(2,1));
  k4(1:3,1)=VV*Xd_tmp(1:3,1)+Vnon*Fb+VP*Xd_tmp(8:10,1)+Cbn*Xd_tmp(17:19,1);
  %k4(4:19,1)=FI*Xd_tmp+GI*Vd_tmp;   
  k4(4:19,1)=FI*Xd_tmp;   
  %%%%%%%%%%%%微分方程的解%%%%%%%%%%%%%%%%%%%%     
  Xd = Xd + (k1 + k2*2 + k3*2 + k4)*T_D/6+GI*Vd_tmp*T_D;         % 一步预测
%   Xd_tmp=Xd;
%   Vd_tmp=Vd;
%   %%%%%%%泰勒展开解微分方程%%%%%%%%
%   XQ=Xd_tmp(4:7,1);
