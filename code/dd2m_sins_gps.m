...
%                           程序设计：熊智  日期：2002/8/9
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [DD2SinsData]=dd2m_sins_gps
%%%%%%%%常数设置%%%%%%%%%%%
...
% PK = zeros(18,18); %协方差阵；
Xd = zeros(19,1);  %DD2滤波器模型状态
PKD = zeros(19,19);%DD2滤波器协方差阵
DD2Xerr =zeros(1,19); %状态估计量的误差值（记录某个时刻的）
...
%%%%%%%%%%%%%%%%%数据记录%%%%%%%%%%%
DD2TraceData=zeros(t_stop/T+1,10);
DD2IMUData=zeros(t_stop/T+1,7);
DD2GPSData=zeros(t_stop+1,7);
DD2SinsData=zeros(t_stop/T+1,10);
DD2KALData=zeros(t_stop+1,20);
DD2ErrData=zeros(t_stop+1,20);
data_i=1;                %数据索引
data_j=1;                %数据索引
...
[Xd,PKD,DD2Xerr]=dd2_gps_init(posiN,atti,Xd,PKD,DD2Xerr);
       %卡尔曼滤波初始化
 %%%%%%%%%%%%%%%%%%%%
  Re=6378137.0;                                      %地球半径（米） 
  f=1/298.257;                                        %地球的椭圆率
  ...
  DD2GPSData(data_i,:)=[t,posiG',veloG'];
  posiG(1)=posiG(1)/(pi/180.0*(Rn+heig)*cos(lati));  
  posiG(2)=posiG(2)/(pi/180.0*(Rm+heig));
  %%%%%%%%%%%%%%%%%%%      
DD2KALData(data_i,:) = [t,DD2Xerr];
DD2ErrData(data_i,:) = [t,Xd'];
data_i=data_i+1;
[vel]=veloN0(atti,veloB);    
%DD2TraceData(data_j,:)=[t,posi',veloB(2,1),atti',vel'];
%%%%%%%%%%%%%%%%%%
  Re=6378137.0;                                      %地球半径（米） 
  f=1/298.257;                                        %地球的椭圆率
  long=posi(1,1)*pi/180.0;lati=posi(2,1)*pi/180.0;heig=posi(3,1);
  ...
  DD2TraceData(data_j,:)=[t,atti',vel',posi'];
  posi(1)=posi(1)/(pi/180.0*(Rn+heig)*cos(lati));
  posi(2)=posi(2)/(pi/180.0*(Rm+heig));
  %%%%%%%%%%%%%%%%%%%%%
  DD2IMUData(data_j,:)=[t,3600.0*Wibb',1/g*Fb'];
  %%%%%%%%%%%%%%%%%%%%
  Re=6378137.0;                                      %地球半径（米） 
  f=1/298.257;                                        %地球的椭圆率
  long=posi(1,1)*pi/180.0;lati=posi(2,1)*pi/180.0;heig=posi(3,1);
  ...
  DD2SinsData(data_j,:)=[t,attiN',veloN',posiN'];
  posiN(1)=posiN(1)/(pi/180.0*(Rn+heig)*cos(lati));
  posiN(2)=posiN(2)/(pi/180.0*(Rm+heig));
  %%%%%%%%%%%%%%%%%%%%
...
while t<=t_stop
  ...
  [Gyro_b,Gyro_r,Gyro_wg,Acc_r,Gyro_z,Acc_z]=imu_err_random(t,T,Gyro_b,Gyro_r,Gyro_wg,Acc_r,Gyro_z,Acc_z);
    %产生随机性误差初始值（此时IMU进入正常工作状态，则随机常数已为常值）
  Fb=Fb+Acc_r+Acc_z;
  Wibb=Wibb+Gyro_b/deg_rad+Gyro_r/deg_rad+Gyro_wg/deg_rad+Gyro_z/deg_rad;  % deg/s
    %加入随机性误差
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%   data_loop=data_loop+1;
  ...
  if( (0.99<T_M)&(T_M<1.01)  )     
      ...
      tic;
      [Xd,PKD,DD2Xerr]=dd2_gps(t,T_D,Fb,Wibb,attiN,veloN,posiN,posiG,veloG,Xd,PKD,DD2Xerr,kflag); 
      fil_time=fil_time+toc;
         %卡尔曼滤波   
      [attiN,veloN,posiN]=dd2_modi(attiN,veloN,posiN,Xd);
       %进行滤波修正
...
disp('%%%%%%%%滤波器工作时间%%%%%%%%');
disp(fil_time);
%dd2_fig(DD2TraceData,DD2IMUData,DD2SinsData,DD2KALData,DD2ErrData,DD2GPSData);
%%%%%%%%%%%%%存储仿真数据%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save dd2trace.dat DD2TraceData -ASCII;    %存储航迹数据
save dd2imu.dat   DD2IMUData   -ASCII;    %存储仿真产生的IMU数据
save dd2sins.dat  DD2SinsData  -ASCII;    %存储导航输出数据
save dd2kal.dat   DD2KALData   -ASCII;    %存储卡尔曼滤波些方差阵
save dd2err.dat   DD2ErrData   -ASCII;    %存储卡尔曼滤波估计的误差修正值
